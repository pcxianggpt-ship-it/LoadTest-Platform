...
import { listExecutions, type TestExecution } from "../api/executions";
...
async function loadDetail() {
  loading.value = true;
  try {
    const [projectData, serverData, datasourceData, taskData, executionData, resultData, reportData] = await Promise.all([
      getProject(projectId.value),
      safe(getJMeterServer(projectId.value)),
      safe(listDatasources(projectId.value)),
      listTasks(projectId.value),
      listExecutions(projectId.value),
      listResults(projectId.value),
  ...
  <section v-loading="loading" class="page-section">
    <div class="page-toolbar">
      ...
      <div class="toolbar-actions">
        ...
        <el-button type="primary" @click="router.push(`/projects/${projectId}/tasks/new`)">创建任务</el-button>
        <el-button type="success" @click="router.push(`/projects/${projectId}/executions`)">执行管理</el-button>
      </div>
    </div>
    ...
    <el-tabs>
        ...
        <el-table :data="tasks" border stripe>
          ...
          <el-table-column label="操作" width="130">
            <template #default="{ row }">
              <el-button link type="primary" @click="router.push(`/projects/${projectId}/executions?taskId=${row.id}`)">执行</el-button>
            </template>
          </el-table-column>
        </el-table>
      </el-tab-pane>

      <el-tab-pane label="执行">
        <el-table :data="executions" border stripe>
          <el-table-column prop="executionName" label="执行名称" min-width="220" />
      ...
      <el-tab-pane label="结果">
        <el-table :data="results" border stripe>
          <el-table-column prop="name" label="结果名称" min-width="180" />
          <el-table-column label="状态" width="130"><template #default="{ row }"><StatusTag :status="row.status" /></template></el-table-column>
          <el-table-column label="操作" width="130"><template #default="{ row }"><el-button link type="primary" @click="router.push(`/results/${row.id}`)">查看</el-button></template></el-table-column>
        </el-table>
      </el-tab-pane>
      ...
      <el-tab-pane label="报告">
        <el-table :data="reports" border stripe>
          ...
          <el-table-column prop="createdAt" label="创建时间" min-width="220" />
          <el-table-column label="操作" width="130"><template #default="{ row }"><el-button link type="primary" @click="router.push(`/reports/${row.id}`)">打开</el-button></template></el-table-column>
        </el-table>
      </el-tab-pane>
    </el-tabs>
  </section>
